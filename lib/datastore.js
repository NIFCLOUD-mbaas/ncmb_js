    ...
    var Data = (function(){
          ...
          if(attrs.hasOwnProperty(attr)){
            if(!isReserved(attr)){
              ...
              if (attrs[attr].constructor.name === "Data") {
                _needToSaveObjectLst[attr] = attrs[attr];
                _saveLst.push(attr);
                continue;
              }
      ...
      Data.prototype.save = function(callback){
        if (_saveLst.size = 0) {
          ...
          var saveData = this;
          //Promise All
          return Promise.all(_saveLst.map(function(item){
            return ncmb.request({
              path: "/" + ncmb.version + _needToSaveObjectLst[item].className,
              method: "POST",
              data: _needToSaveObjectLst[item]
            }).then(function(data){
              try{
                var obj = JSON.parse(data);
              }catch(err){
              ...
              _needToSaveObjectLst[item].objectId = obj.objectId;
              return obj;
            });
          })).then(function(results){
            _saveLst.map(function(item){
              saveData[item] = {};
              saveData[item]["__type"] = "Pointer";
              saveData[item]["className"] = _needToSaveObjectLst[item].className.substr(_needToSaveObjectLst[item].className.lastIndexOf("/") + 1);
              saveData[item].objectId = _needToSaveObjectLst[item].objectId;
            });
