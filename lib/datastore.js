  ...
  function DataStore(name, options){
    ...
    var Data = (function(){
      ...
      Data.prototype.save = function(callback){
        ...
        return Promise.all(this.saveList.map(function(obj){
          return save(obj.object);
        }))
        .then(function(data){
          for(var i = 0; i < this.saveList.length; i++){
            var obj = this.saveList[i];
            setPointer(this, obj.key, obj.object.className.slice(1), obj.object.objectId);
          }
          delete this.saveList;
          return save(this, callback);
        }.bind(this))
        .catch(function(err){
          throw err;
      ...
      Data.prototype.set = function(key, value){
        ...
        if(!value.objectId){
          ...
          var unsavedObj = {key: key, object: value}
          this.saveList.push(unsavedObj);
          return;
        }
        setPointer(this, key, value.className.slice(1), value.objectId);
      };

      var setPointer = function(object, key, className, objectId){
        if (className === "users") className = "user";
        if (className === "files") className = "file";
