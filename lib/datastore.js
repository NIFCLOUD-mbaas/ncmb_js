  ...
  function DataStore(name, options){
    ...
    var Data = (function(){
      ...
      Data.saveAll = function(list, callback){
        ...
        if (list) {
          var i = 0;
          while (i*maxRequestBatch < list.length) {
              arySave[i] = list.slice(i*maxRequestBatch , i*maxRequestBatch+maxRequestBatch);
              i++;
          }
        }
        function _createSaveAllRequestStr(reqlist){
          var _i;
          var _requestString = '{"requests":[';
          var _dataToSave = {};
          for (_i = 0; _i < reqlist.length; _i++) {
            var _object = reqlist[_i];
            ...
            if (_i < (reqlist.length - 1)) {
              _requestString += ','; 
            }
          }
          _requestString += ']}';
          return JSON.parse(_requestString);
        }
        ...
        .then(function(data){
          for (var _j = 0; _j<data.length; _j++ ) {
            var obj = JSON.parse(data[_j]);
            for (var _k = 0; _k<obj.length; _k++ ) {
              if(obj[_k].success && obj[_k].success.objectId) {
                list[_j*maxRequestBatch+_k].objectId = obj[_k].success.objectId;
                continue;
              }
            }
          }
          if(callback) return callback(null, list);
          return list;
        })
        .catch(function(err){
