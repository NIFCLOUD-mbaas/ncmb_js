...
/**
* 更新時に、既に保存されている値に対して操作を行うオペランドを扱うクラスです。（set, getは更新時以外も利用可能。）
* DataStore, User, Roleクラスから呼び出し、それぞれのプロトタイプメソッドとして利用します。
* 
* @class Operation
* @constructor
*/
var Operation = module.exports = (function(){
  ...
  * @param {number} amount 更新時の増加量。指定しない場合は1が設定される。負の数も指定可能
  * @return this
  */
  ...
  * @method add
  * @param {string} key 処理を設定したいキー
  * @param  objects 配列に追加したい値。配列での複数指定、およびaddの複数回実行による追加も可能。
  * @return this
  */
  ...
  * @method addUnique
  * @param {string} key 処理を設定したいキー
  * @param  objects 配列に追加したい値。配列での複数指定、およびaddの複数回実行による追加も可能。既に追加した値を追加しようとした場合エラーが返る
  * @return this
  */
  ...
  * @method remove
  * @param {string} key 処理を設定したいキー
  * @param  objects 配列から削除したい値。配列での複数指定、およびaddの複数回実行による追加も可能
  * @return this
  */
