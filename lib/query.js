...
var Query = module.exports = (function(){
  ...
  Query.prototype.in                   = function(key, value){
    if(!(value instanceof Array)) throw new this.ncmb.Errors.InvalidArgumentError();
  ...
  Query.prototype.exists               = function(key, value){
    if(typeof value !== "boolean") throw new this.ncmb.Errors.InvalidArgumentError();
    return setOperand(this, key, value, "$exists");};
  Query.prototype.regularExpressionTo  = function(key, value){
    if(typeof value !== "string") throw new this.ncmb.Errors.InvalidArgumentError();
  ...
  Query.prototype.near  = function(key, location){
    if(!location.className || location.className !== "/geolocation"){
  ...
  Query.prototype.withinSquare = function(key, southWestVertex, northEastVertex){
    if(!southWestVertex.className || southWestVertex.className !== "/geolocation" || !northEastVertex.className || northEastVertex.className !== "/geolocation"){
      throw new this.ncmb.Errors.InvalidArgumentError();
    }
  ...
  Query.prototype.or = function(subqueries){
    ...
    for(var i = 0; i < subqueries.length; i++){
      if(!subqueries[i]._where) throw new this.ncmb.Errors.InvalidArgumentError();
  ...
  Query.prototype.relatedTo = function(object, key){
    ...
    if(!object.objectId){
      throw new this.ncmb.Errors.NoObjectIdError();
    }
    if(object.className === "/users"){
      className = "user";
    }else if(object.className === "/roles"){
      className = "role";
    }else{
  ...
  Query.prototype.include = function(key){
    if(typeof key !== "string") throw new this.ncmb.Errors.InvalidArgumentError();
    if(!this._include){
      this._include = key;
    }else{
      this._include = this._include + "," + key;
    }
    console.log("include",this._include);
    return this;
  };
  ...
  Query.prototype.limit = function(limit){
    ...
    if(limit < 1 || limit >1000){
      throw new this.ncmb.Errors.InvalidLimitError("limit は 1~1000 の範囲で指定して下さい");
    }
    this._limit = limit;
    return this;
  };
  Query.prototype.skip = function(skip){
    if(typeof skip !== "number") throw new this.ncmb.Errors.InvalidskipError("skip は number で指定して下さい");
    if(skip < 0) throw new this.ncmb.Errors.InvalidskipError("skip は 0以上で指定して下さい");
    this._skip = skip;
    return this;
  };
