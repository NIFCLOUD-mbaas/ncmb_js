...
var Query = module.exports = (function(){
  ...
  Query.prototype.in                   = function(key, value){
    if(!Array.isArray(value)) throw new Errors.InvalidArgumentError();
  ...
  Query.prototype.exists               = function(key, value){
    if(typeof value !== "boolean") throw new Errors.InvalidArgumentError();
    return setOperand(this, key, value, "$exists");};
  Query.prototype.regularExpressionTo  = function(key, value){
    if(typeof value !== "string") throw new Errors.InvalidArgumentError();
  ...
  Query.prototype.or = function(subqueries){
    ...
    for(var i = 0; i < subqueries.length; i++){
      if(!subqueries[i]._where) throw new Errors.InvalidArgumentError();
  ...
  Query.prototype.select = function(key, subkey, subquery){
    ...
    if(!subquery._className) throw new Errors.InvalidArgumentError();
    var className = null;
  ...
  Query.prototype.limit = function(limit){
    ...
    if(limit < 1 || limit >1000){
      throw new Errors.InvalidLimitError("limit は 1~1000 の範囲で指定して下さい");
    }
    this._limit = limit;
    return this;
  };
  Query.prototype.skip = function(skip){
    if(typeof skip !== "number") throw new Errors.InvalidskipError("skip は number で指定して下さい");
    if(skip < 0) throw new Errors.InvalidskipError("skip は 0以上で指定して下さい");
    this._skip = skip;
    return this;
  };
