...
var setPointer = function(object, key, className, objectId){
  ...
  if(className === "/roles") className = "role";
...
var setPointerIntoArray = function(array, className, objectId){
  if(className === "/users") className = "user";
  if(className === "/roles") className = "role";
  if(className.indexOf("/classes/") !== -1){
    className = className.slice(9);
  }
  var pointer = {__type: "Pointer", className: className, objectId: objectId};
  array.push(pointer);
  return pointer;
};
  ...
  return Promise.all(dataArray.map(function(elem){
    ...
    if(obj instanceof ncmb.Relation) {
      ...
      return Promise.all(obj.objects.map(function(elems){
        if(elems.objectId){
          return setPointerIntoArray(savedArray, elems.className, elems.objectId);
        }
        if(elems instanceof ncmb.User){
          return elems.signUpByAccount()
                      .then(function(res){
                        console.log("res of elems save", res);
                        return setPointerIntoArray(savedArray, elems.className, elems.objectId);
                      });
        }
        return elems.save()
      ...
      .then(function(res){
        ...
        console.log("saved relation", JSON.stringify(obj));
        data[key] = obj;
        return data[key];
      });
    }
