...
var setPointer = function(object, key, obj, ncmb){
  var className = null;
  ...
  object[key] = {__type: "Pointer", className: className, objectId: obj.objectId};
  return object[key];
};

var setPointerIntoArray = function(array, elem, ncmb){
  var className = null;
  if(elem instanceof ncmb.User){
    className = "user";
  }else if(elem instanceof ncmb.Role){
    className = "role";
  }else if(elem.className.indexOf("/classes/") !== -1){
    className = elem.className.slice(9);
  }
  var pointer = {__type: "Pointer", className: className, objectId: elem.objectId};
  array.push(pointer);
  return pointer;
};
...
var presavePointerObjects = function(ncmb, data){
  ...
  return Promise.all(dataArray.map(function(elem){
    ...
    if(obj instanceof ncmb.Relation) {
      var savedArray = [];
      return Promise.all(obj.objects.map(function(elems){
        if(elems.objectId){
          return setPointerIntoArray(savedArray, elems, ncmb);
        }
        if(elems instanceof ncmb.User) elems.save = elems.signUpByAccount.bind(elems);
        return elems.save()
                    .then(function(res){
                      return setPointerIntoArray(savedArray, elems, ncmb);
                    });
    ...
    if(obj.objectId){
      return setPointer(data, key, obj, ncmb);
    }

    if(obj instanceof ncmb.User) obj.save = obj.signUpByAccount.bind(obj);
    return obj.save()
              .then(function(res){
                return setPointer(data, key, obj, ncmb);
              });
