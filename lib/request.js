...
var presavePointerObjects = function(data, ncmb){
  ...
  for (var element in data){
    if(data[element].__type && data[element].__type === "Relation"){
          delete data[element]
    }
  }
...
module.exports = function(opts, callback){
  ...
  var headers = {
    ...
    "X-NCMB-SDK-Version":        "javascript-2.2.0"
  };
  ...
  return new Promise(function(resolve, reject){
    var _callback = function(err, res){
      if(err && err.response && err.response.error){
        // Since error content is combined by properties 
        // when printing by console.log() or alert(JSON.stringify()),
        // remove unnecessary error's properties
        if(err.response.error.method){
          delete err.response.error.method;
        }
        if(err.response.error.url){
          delete err.response.error.url;
        }
        if(err.response.error.status){
          delete err.response.error.status;
        }
        // Use code property, error property when printing log by alert(JSON.stringify())
        err.response.error.code = err.response.body.code;
        err.response.error.error = err.response.body.error;
        // Use message property when printing log by console.log()
        err.response.error.message = err.response.text;
