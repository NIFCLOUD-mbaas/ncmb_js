...
var User = module.exports = function(ncmb){
  var reserved = [
    ...
    "signUpByAccount", "signUpWith",
    "login", "loginAsAnonymous", "loginWithMailAddress",
    ...
    if(provider instanceof ncmb.User){
      ...
      if(Object.keys(user.authData).length === 1){
        ...
        if(typeof data === "function"){
          return ( callback || Promise.reject.bind(Promise))(new Errors.NoProviderDataError());
        }else{
          provider = data;
          data = null;
        }
  ...
  User.prototype.loginWith = function(provider, data, callback){
    ...
    if (this.authData){
      if(Object.keys(this.authData).length !== 1){
        if(provider === null || !this.authData[provider]){
          return ( callback || Promise.reject.bind(Promise))(new Errors.NoProviderDataError());
        } 
        data = this.authData[provider];
      }else if(this.authData.facebook){
         provider = "facebook";
         data = this.authData.facebook;
      }else if (this.authData.twitter) {
        provider = "twitter";
        data = this.authData.twitter;
      }else if (this.authData.google) {
        provider = "google";
        data = this.authData.google;
      }else {
        return ( callback || Promise.reject.bind(Promise))(new Errors.InvalidAuthDataError());
      };
    };
    if (! data) {
    ...
    switch(provider){
      case "facebook":
        if(!data.id || !data.access_token || !data.expiration_date){
          return ( callback || Promise.reject.bind(Promise))(new Errors.InvalidOAuthDataError());
        }
        break;
      case "twitter":
        if(!data.id || !data.screen_name || !data.oauth_consumer_key|| !data.consumer_secret || !data.oauth_token || !data.oauth_token_secret){
          return ( callback || Promise.reject.bind(Promise))(new Errors.InvalidOAuthDataError());
        }
        break;
      case "google":
        ...
        break;
      case undefined:
        return ( callback || Promise.reject.bind(Promise))(new Errors.NoProviderError());
        break;
      default :
    ...
    oauthData.authData[provider] = data;
    return ncmb.request({
