...
var User = module.exports = function(ncmb){
  ...
  * @return {boolean} メールアドレス確認値が未存在場合はnull/確認済みの場合はtrue/未確認の場合はfalse
  */
  User.prototype.isMailAddressConfirmed = function(){
    //メールアドレス確認値を取得判断
    var isMailAddressConfirm = this.mailAddressConfirm;
   	if(null == isMailAddressConfirm){
		return null;
	}else{
		return isMailAddressConfirm;
	}
  };
