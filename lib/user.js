...
var User = module.exports = function(ncmb){
  ...
  */
  User.prototype.delete = function(callback){
    ...
    var currentUserId = User.getCurrentUser().objectId;
    return ncmb.request({
      path: "/" + ncmb.version + this.className + "/" + this.objectId,
      method: "DEL"
    }).then(function(){
  ...
  * すでにセッショントークンを保持している場合、更新処理は行いません。
  * セッショントークンの期限切れが発生している場合、一度ログアウトしてから再度ログインしてください。
  * Phương thức này được sử dụng để đăng nhập với tài khoản mạng xã hội, kết quả trả về là thông tin của user đã được đăng nhập.
  *
  ...
  */
  ...
  /**
   * Phương thức dùng để liên kết người dùng với tài khoản mạng xã hội như facebook/google/twiter/apple
   * 
   * @method linkWith
   * @param {string} provider 連携するサービスプロバイダ名 facebook/twitter/google/apple
   * @param {Object} data 認証に必要な情報を保持したJSON形式のオブジェクト
  ...
  /**
   * Phương thức dùng để ngắt liên kết người dùng với tài khoản mạng xã hội như facebook/google/twiter/apple
   * 
   * @method unLinkWith
   * @param {string} provider 連携するサービスプロバイダ名 facebook/twitter/google/apple
   * @param {function} callback コールバック関数
