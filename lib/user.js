...
*
* ※注意：以下のItem Indexには対応しているメソッド一覧が記載されています。
* メソッドにはインスタンス用メソッド Instance methodとスタティックメソッド Static methodの２種類があります。
* それぞれ利用時のメソッドとリファレンス上の表記が異なりますので、下記をご参照の上ご利用ください。
*
*   - NCMB.User#メソッド名: インスタンス用メソッド Instance method
*      - 例：NCMB.User#login の利用例
*        ```
*        var user = new ncmb.User({userName:"Yamada Tarou", password:"password"});
*        user.login();
*        ```
*   - NCMB.UserConstructor#メソッド名: スタティックメソッド Static method
*      - 例：NCMB.UserConstructor#login　の利用例
*        ```
