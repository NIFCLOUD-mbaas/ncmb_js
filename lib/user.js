...
var User = module.exports = function(ncmb){
  ...
  User.login = function(user, callback){
    ...
    return ncmb.request({
      ...
      try{
        ...
        setItem(SESSION_TOKEN_PATH, ncmb.sessionToken);
      }catch(err){
        throw err;
      }
  ...
  User.logout = function(callback){
    ...
    console.log("token:" + this.getLocalSessionToken());
    console.log("ncmbToken:" + ncmb.sessionToken);

    if(!this.getCurrentUser() || !this.getLocalSessionToken() || !ncmb.sessionToken){
      removeItem(CURRENT_USER_PATH);
      removeItem(SESSION_TOKEN_PATH);
      delete ncmb.sessionToken;
      return (callback || Promise.reject.bind(Promise))(new Errors.NoLoginError());
    }
  ...
  User.getCurrentUser = function(){
    try{
      var obj = JSON.parse(getItem(CURRENT_USER_PATH));
    }catch(err){
      throw err;
    }
    return obj;
  ...
  User.getLocalSessionToken = function(){
    return getItem(SESSION_TOKEN_PATH);
  };
  ...
  var removeItem = function(key){
    try{
      localStorage.removeItem(makeStoragePath(key, ncmb.apikey));
    }catch(err){
      throw err;
    }
  };
